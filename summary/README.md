...
This is the single, self-contained report for the whole project. It consolidates the results of
**14 experiment runs**, with every figure embedded inline below so you can read the full story
without opening a notebook. Everything here is regenerated by
[`00_summary.ipynb`](00_summary.ipynb) (underlying tables in [`data/`](data/), source figures in
[`figures/`](figures/)).
...
### 3.1 Whole-gender baselines (Q1)

![Whole-gender demand elasticity for men-8 and women-8 across the five control specifications, text+image](figures/01_whole_gender_specs.png)
...
### 3.2 Within-gender heterogeneity (Q2)

![Subcategory demand elasticities within each gender, ordered most to least elastic; fashion sneakers most elastic in both](figures/02_heterogeneity.png)
...
### 3.3 The core experiment: lazy vs proper (Q3)

![Lazy vs proper demand elasticity per subcategory with 90% confidence intervals; the intervals overlap in every subcategory](figures/03_lazy_vs_proper.png)

Main spec, text+image, demand elasticity (table also in [`data/lazy_vs_proper.csv`](data/lazy_vs_proper.csv)):
...
### 3.4 Robustness across specifications (Q4)

![Elasticity estimates across the five control specifications per experiment; whole-gender and women subcats are stable, men subcats swing](figures/04_spec_robustness.png)
...
### 3.5 Does vision help? (Q5)

![Text-only vs text+image demand elasticity per experiment; the two variants track closely](figures/05_txt_vs_txtimg.png)
...
### 3.6 Heterogeneous treatment effects (Q6)

![CATE means and ranges per experiment; within-experiment heterogeneity is present but mostly not statistically significant](figures/06_cate_ranges.png)

CATE means/ranges (table also in [`data/cate_het.csv`](data/cate_het.csv)) show within-experiment variation in the elasticity, but the χ²
...
### 3.7 Sample size and reliability

![Confidence-interval width vs number of training products; CIs shrink sharply with sample size](figures/07_sample_size_reliability.png)
